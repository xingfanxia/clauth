...
fn all_exhausted_shows_resumes_hint_under_any_selected_member() {
    ...
    let on_a = member_detail(&cfg, "a", 0, 2, false, 0, false, None, None, None, 60, None);
    ...
    let on_b = member_detail(&cfg, "b", 1, 2, false, 0, false, None, None, None, 60, None);
...
fn partially_exhausted_chain_hides_resumes_hint() {
    ...
    let lines = member_detail(&cfg, "a", 0, 2, false, 0, false, None, None, None, 60, None);
    assert!(
...
fn last_resort_hint_wraps_on_a_narrow_pane() {
    ...
    let lines = member_detail(&cfg, "a", 0, 2, true, 1, false, None, None, None, 28, None);
...
fn last_resort_hint_names_the_currently_marked_member() {
    ...
    let lines = member_detail(&cfg, "a", 0, 2, true, 4, false, None, None, None, 80, None);
    let hint = lines
        .iter()
        .map(line_text)
        .find(|t| t.contains("└"))
...
fn last_resort_value_aligns_with_other_rows() {
    ...
    let texts: Vec<String> =
        member_detail(&cfg, "a", 0, 1, true, 1, false, None, None, None, 60, None)
            .iter()
            .map(line_text)
            .collect();
...
fn max_spend_row_renders_off_at_zero_and_dollars_when_set() {
    ...
    let row = |c: &crate::profile::AppConfig| -> String {
        member_detail(c, "a", 0, 1, true, 5, false, None, None, None, 60, None)
            .iter()
            .map(line_text)
            .find(|t| t.contains("max spend"))
            .expect("max spend row")
...
fn blocked_member_shows_the_worst_reason_pill() {
    ...
    let lines = member_detail(&cfg, "a", 0, 1, false, 0, false, None, None, None, 60, None);
...
fn max_spend_dims_when_spend_budget_is_off() {
    ...
    let off = member_detail(&cfg, "a", 0, 1, true, 4, false, None, None, None, 60, None);
    let off_val = off
        .iter()
    ...
    let on = member_detail(&cfg, "a", 0, 1, true, 4, false, None, None, None, 60, None);
    let on_val = on
        .iter()
        .find_map(|l| span_style(l, "$25.00"))
        .expect("max spend ceiling renders");
