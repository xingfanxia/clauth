...
- **Automatic token refresh**: OAuth refresh tokens are single-use, so rotation stays lazy: a stale access token rotates the moment a usage query 401s. The **active** account on macOS shares its single-use chain with the running `claude`, and whoever refreshes first revokes the other side — so clauth never bets on winning that race. When Claude Code rotates first, clauth **adopts** CC's fresher pair from its file mirror (identity-guarded: a login belonging to a different account is never captured unattended) instead of spending a revoked refresh token; when clauth rotates, it mirrors the fresh pair straight into the Keychain so the running `claude` never lapses (rotation coherence, #1). An opt-in **preemptive rotation** toggle (Config tab, off by default) additionally rotates the active account a few poll intervals ahead of expiry — an optimization that makes adopt events rarer, not a correctness mechanism. <kbd>t</kbd> force-rotates every account.
