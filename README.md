...
- **Sign in a new account**: `clauth login <profile> [--model <id>]` opens your browser for a real Claude Code OAuth sign-in (the same PKCE flow Claude Code uses) and writes the minted tokens straight into a new profile, without touching the session you're already signed into. Works identically on Linux, macOS, and Windows — unlike running Claude Code's own `/login`, which on macOS lands only in a per-config-dir Keychain item and leaves the profile empty. `--model` sets the new profile's default model (a preset alias or a full model id).
