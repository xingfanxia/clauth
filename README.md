...
- **Sign in an account**: `clauth login <profile> [--model <id>]` opens your browser for a real Claude Code OAuth sign-in (the same PKCE flow Claude Code uses) and writes the minted tokens straight into a new profile, without touching the session you're already signed into. Pass an existing profile name to re-authenticate it in place; clauth asks to confirm before replacing that profile's saved login. Works identically on every desktop platform (Linux, macOS, Windows), unlike running Claude Code's own `/login`, which on macOS lands only in a per-config-dir Keychain item and leaves the profile empty. `--model` sets the profile's default model (a preset alias or a full model id).
...
- **Token usage dashboard**: the Tokens tab reads Claude Code's own token history (the stats cache, topped up from live session transcripts). It rolls that into per-model totals with a today panel, daily peak, busiest hour, and usage sparklines. Press <kbd>c</kbd> to count cache reads/writes in the totals; models past 1M tokens break out on their own. The <kbd>a</kbd> actions menu narrows the model bars and the per-model breakdown to Claude models only, or to everything else.
...
- **Automatic token refresh**: OAuth refresh tokens are single-use, so rotation stays lazy for every account except one. A stale access token rotates the moment a usage query 401s; the **active** account on macOS instead rotates ~30 min *ahead* of expiry and mirrors the fresh pair straight into the Keychain — the running `claude` shares that single-use chain, and whoever refreshes first revokes the other side, so clauth deliberately wins the race and keeps the live login permanently fresh (rotation coherence, #1). <kbd>t</kbd> force-rotates every account.
...
- Mark a member **last resort** (a toggle row on its Fallback card) to make it the chain's parking spot: chosen only when every other member is past its threshold, never switched away from. The chain has one parking spot — marking a member clears the mark on the rest. Claude Code then surfaces its own *"out of 5h limit"* message once that account also runs out. The threshold itself only means "switch away at N%" — a 100% threshold is just a late switch point.
