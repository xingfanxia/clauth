...
- **Delete an account**: `clauth delete <profile> [--yes]` removes a profile and all its credentials (OAuth tokens or API key, caches, the on-disk profile dir; a deleted active account is also unwired from live `~/.claude`). Confirms `[y/N]` on a TTY unless `--yes`. Delete is irreversible, so a non-TTY run must pass `--yes`; it never deletes unprompted.
