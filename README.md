...
- **Sign in a new account**: `clauth login <profile> [--model <id>]` opens Claude Code's own `/login` inside that profile's runtime and saves the completed login into the profile, without touching the session you're already signed into. `--model` sets the profile's default model (a preset alias or a full model id) before the session starts.
