...
- **Delete an account**: `clauth delete <profile> [--yes]` removes a profile and all its credentials (OAuth tokens or API key, caches, the on-disk profile dir). Confirms `[y/N]` on a TTY unless `--yes`; a non-TTY stdin skips the prompt so scripts can delete.
