...
- **Feed the session token**: `clauth feed <profile> on` points the profile's `session-token.json` at its own clauth-private usage chain instead of a static mint — the daemon re-stamps it with that chain's current access token on every rotation and on a freshness timer, carrying the full scope set and the account's `subscriptionType` but **no refresh token**. That keeps the split's whole point (sessions hold nothing rotatable, so they can never race the refresher) while giving them a bearer the API recognizes as the plan it is, so plan-gated models work in a clauth-managed session — a `claude setup-token` mint carries neither `user:profile` nor a subscription stamp and gets capped. The mint it supersedes is preserved at `session-token.static.json`; `clauth feed <profile> off`, or a terminally dead usage chain, restores it rather than signing sessions out. The Setup tab's `token` row switches to an hours-scale `fed · refreshes in ~Nh` countdown (routine maintenance, not a dying credential) and reads `feed stalled` if the feeder ever stops.
