...
/// Stderr spinner shown while a one-shot CLI step runs. Silent when stderr is
/// not a terminal so piped/redirected output stays free of control codes. On
/// drop it stops the thread, joins it, and clears its line — so it never
/// outlives the work it wraps or leaks frames into a child's inherited stderr.
struct Spinner {
    stop: Arc<AtomicBool>,
