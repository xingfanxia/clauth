...
/// `profiles` array at all — every current call site passes `false` (hidden by
/// default); a future `--all`/`--disabled` CLI flag is the only intended way to
/// flip it to `true`. The active profile is ALWAYS kept, disabled or not: the
