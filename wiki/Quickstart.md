...
| `clauth use-reset <profile>` | `--list`, `--yes` | spend one of a codex account's banked usage-limit resets; `--list` shows them and spends nothing ([Codex](Codex#use-a-usage-limit-reset)) |
...
- **`delete`, `disable` and `use-reset` want a TTY.** Each prompts `[y/N]`; on a non-TTY stdin they refuse unless you pass `--yes`. `--force` is the only way past `delete`'s live-session guard, and `--yes` alone does not override it.
- **Bare names span both rosters.** `clauth <name>`, `delete` and `start` try the Claude Code profiles first, then the codex ones; an unknown name lists both (`available: … · codex: …`). `disable`, `enable`, `rolling-token` and `static-token` refuse a codex name as one ([Codex](Codex)), and `use-reset` refuses a Claude Code name the same way.
