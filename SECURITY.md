...
| `~/.clauth/profiles/<name>/session-token.static.json` | the `claude setup-token` mint the session feed superseded, kept so `clauth feed <p> off` (or a dead chain) can restore it | file `0600`, dirs `0700` |
| `~/.clauth/quarantine/<ts>-<seq>-<name>.session-token.json` | a mis-filled sidecar (one carrying a refresh token) moved aside before repair, kept as evidence | file `0600`, dir `0700` |
